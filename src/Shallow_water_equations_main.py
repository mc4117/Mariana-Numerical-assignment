...
def main():
    ...
    fig1_implicit_multiplec, fig2_implicit_multiplec, ax1_implicit_multiplec, ax2_implicit_multiplec = pltfns.plot_multiple_c(ic.initialconditions_cosbell, nm.A_grid_implicit_method, crange, colorrange_multic)
    ...
    # and h is zero everywhere apart from one point at the centre where it is one
    fig1_implicit, fig2_implicit, ax1_implicit, ax2_implicit = pltfns.plot_multiple_iterations(ic.initialconditions_spike, nx_adapted, nt_adapted, number_plotted, nm.A_grid_implicit_method, plotparameterrange)
    ...
    # However a von-neumann stability analysis shows this is unstable for c > 1 therefore try the following implicit method
    
    # plot solution at various time iterations for a implicit method on a staggered grid for the initial condition where u is zero everywhere 
    # and h is zero everywhere apart from one point at the centre where it is one
    fig1_C_grid_implicit, fig2_C_grid_implicit, ax1_C_grid_implicit, ax2_C_grid_implicit = pltfns.plot_multiple_iterations(ic.initialconditions_spike, nx_adapted, nt_adapted, number_plotted, nm.C_grid_implicit_method, plotparameterrange, staggered = True)
    ...
    fig1_error, fig2_error, ax1_error, ax2_error, error_norms_u, error_norms_h = errfns.error_fn(nx_1, nt_1, xmin_1, xmax_1, H, g, c)
    ...
    dx_list, dt_list, norm_A_grid_listu, norm_A_grid_listh, norm_C_grid_listu, norm_C_grid_listh, norm_A_grid_implicit_listu, norm_A_grid_implicit_listh, norm_C_grid_implicit_listu, norm_C_grid_implicit_listh = errfns.error_fn_cossin(nx_range, nt_range, total_time, xmin = -math.pi, xmax = math.pi, H = 1, g = 1, c = 0.1)
    ...
    cossin_gradient_A_grid_implicit_u_dx = np.polyfit(np.log(dx_list), np.log(norm_A_grid_implicit_listu),1)[0]
    cossin_gradient_C_grid_implicit_u_dx = np.polyfit(np.log(dx_list), np.log(norm_C_grid_implicit_listu),1)[0]
    ...
    cossin_gradient_A_grid_implicit_h_dx = np.polyfit(np.log(dx_list), np.log(norm_A_grid_implicit_listh),1)[0]
    cossin_gradient_C_grid_implicit_h_dx = np.polyfit(np.log(dx_list), np.log(norm_C_grid_implicit_listh),1)[0]
    ...
    cossin_gradient_A_grid_implicit_u_dt = np.polyfit(np.log(dt_list), np.log(norm_A_grid_implicit_listu),1)[0]
    cossin_gradient_C_grid_implicit_u_dt = np.polyfit(np.log(dt_list), np.log(norm_C_grid_implicit_listu),1)[0]
    ...
    cossin_gradient_A_grid_implicit_h_dt = np.polyfit(np.log(dt_list), np.log(norm_A_grid_implicit_listh),1)[0]
    cossin_gradient_C_grid_implicit_h_dt = np.polyfit(np.log(dt_list), np.log(norm_C_grid_implicit_listh),1)[0]
    ...
    print("A_grid implicit|%f | %f | %f | %f" % (cossin_gradient_A_grid_implicit_u_dx, cossin_gradient_A_grid_implicit_u_dt, cossin_gradient_A_grid_implicit_h_dx, cossin_gradient_A_grid_implicit_h_dt))
    print("C_grid implicit|%f | %f | %f | %f" % (cossin_gradient_C_grid_implicit_u_dx, cossin_gradient_C_grid_implicit_u_dt, cossin_gradient_C_grid_implicit_h_dx, cossin_gradient_C_grid_implicit_h_dt))
